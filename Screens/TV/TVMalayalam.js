...
export default function TVMalayalam({navigation}) {
  ...
  async function fetchData() {
    setRefreshing(true);
    try {
      const response = await axios.get(`https://movierulz.vercel.app/malayalam/${pageno}`);
  ...
  function h(index) {
    if (index === data.length - 2) {
      if (pageno === 1) {
        return (
          <View style={{ position:'absolute',bottom:80,width:'100%',left:"-90%" }}>
        ...
        return (
          <View style={{ position:'absolute',bottom:30,width:'100%',left:"-90%" }}>
  ...
  const renderRow = ({ item, index }) => {
      return (
        ...
        {index>=data.length-2? <View style={{width:"100%",height:tv?150:40}} ></View> : null}
        {h(index)}
        </View>
      );
