...
export default function TestVlc({navigation,route}) {
  ...
  return (
    <View style={{ flex: 1,width:ORE?"100%":"100%",height:"100%", justifyContent: 'center', alignItems: 'center', backgroundColor: 'black', }}  >
      {/* <StatusBar hidden={false} showHideTransition={'fade'} translucent={true} animated={true} barStyle={'dark-content'}  /> */}
      <View style={{width:"100%",height:"100%",justifyContent:"center",alignItems:"center",transform:[{rotate:ORE?"90deg":"0deg"}]}}
      ...
      <VLCPlayer
        paused={play}
        style={{ width: ORE?'220%':"100%", height: isTv?"100%":ORE ? '42%' : '30%',zIndex:-1 }}
        source={{
          uri:
      ...
      {timmingStat?<View
        style={{
          ...
          <View style={{position:"absolute",width:ORE?Dimensions.get('screen').height-40:"100%",height:"auto",top:ORE?"50%":"3%",right:ORE?-Dimensions.get('screen').width+0:"0%",justifyContent:"center",alignItems:"center",transform:[{rotate:ORE?"90deg":"0deg"}]}} >
            <Pressable style={{backgroundColor: 'rgba(150,30,30,.4)', height: 'auto', width: 'auto', borderRadius: 100,position:"absolute",left:ORE?".5%":"3%"}}
            onPress={()=>{
              setPlay(true)
          ...
          <Text style={{color:"white",width:"40%",backgroundColor:"rgba(150,30,30,.4)",textAlign:"center",borderRadius:10}} >{title}</Text>
            ...
            style={{ backgroundColor: 'rgba(150,30,30,.4)', height: 'auto', width: 'auto', borderRadius: 100,position:"absolute",right:"3%",top:"0%",}}
            >
          ...
          {BufferRate==100?null:<Text style={{color:"white",backgroundColor:"rgba(150,30,30,.4)",padding:5,borderRadius:10,transform:[{rotate:ORE?"90deg":"0deg"},]}} >{Math.floor(BufferRate)}%</Text>}
          {Math.floor(((currentTime))/1000)>9?
          <Pressable style={{ backgroundColor: 'rgba(150,30,30,.4)', height: 'auto', width: 'auto', borderRadius: 100,position:"absolute",top:ORE?"30%":"47.5%",left:ORE?"45%":"20%",transform:[{rotate:ORE?"90deg":"0deg"}] }}
        ...
        <Pressable onPress={handlePlay} style={{ backgroundColor: 'rgba(150,30,30,.4)', height: 'auto', width: 'auto', borderRadius: 100,transform:[{rotate:ORE?"90deg":"0deg"}] }}>
        ...
        <Pressable style={{ backgroundColor: 'rgba(150,30,30,.4)', height: 'auto', width: 'auto', borderRadius: 100,position:"absolute",top:ORE?"67%":"47.5%",right:ORE?"45%":"20%",transform:[{rotate:ORE?"90deg":"0deg"}] }}
        onPress={()=>{
          // setSeekValue(currentTime-10)
          setSeekValue(Math.floor(((currentTime))/1000)+10)
        }}
        ...
        <Pressable
          style={{
            ...
            borderRadius: 100,
            transform:[{rotate:ORE?"90deg":"0deg"}]
        ...
        <View style={{width:ORE?Dimensions.get('screen').height-40:"100%",height:ORE?"auto":"auto",position:"absolute",
        top:isTv?"76%":ORE?"48%":"90%",
        // top:"50%",
        left:ORE?"-90d%":"0%",
        transform:[{rotate:ORE?"90deg":"0deg"}]
        }}>
      ...
      {popStatus?<Animated.View
        ...
        >
          ...
          <View style={{width:ORE?"100%":"100%",height:"90%",position:"absolute",bottom:"0%",borderRadius:20,justifyContent:"center",alignItems:"center",}} >
            <View style={{width:"50%",height:"90%",position:"absolute",left:"3%",justifyContent:"center",alignItems:"center",}} >
          <Text style={{color:"white"}} >Audio</Text>
          <AudioTracks data={audioData} />
          </View>
          <View style={{width:"50%",height:"90%",position:"absolute",right:"3%",justifyContent:"center",alignItems:"center",}} >
